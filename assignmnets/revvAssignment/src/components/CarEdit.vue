...
async function save() {
  ...
  try {
    if (newImageFile.value) {
      ...
      await axios.put(`http://localhost:5058/api/Car/${car.value.id}`, formData, {
        headers: {
      ...
      // No image changed → send pure JSON
      const payload = {
        ...car.value,
        userId: user.email,
      ...
      await axios.put(`http://localhost:5058/api/Car`, payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
    }
